...
public class RtmpConnection implements RtmpPublisher {
    ...
    public boolean connect(String url) {
        ...
        // socket connection
        FullLog.LogD(TAG, "connect() called. Host: "
                + host
                + ", port: "
                + port
                + ", appName: "
        ...
        try {
            ...
            FullLog.LogD(TAG, "connect(): socket connection established, doing handhake...");
            handshake(inputStream, outputStream);
            FullLog.LogD(TAG, "connect(): handshake done");
        } catch (IOException e) {
            Log.e(TAG, "Error", e);
        ...
        rxPacketHandler = new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                FullLog.LogD(TAG, "starting main rx handler loop");
                handleRxPacketLoop();
            }
        });
    ...
    private boolean rtmpConnect() {
        ...
        if (user != null && password != null) {
            ...
            FullLog.LogD(TAG, "rtmpConnect(): Building 'connect' invoke packet");
    ...
    private boolean createStream() {
        ...
        FullLog.LogD(TAG, "createStream(): Sending releaseStream command...");
        // transactionId == 2
        ...
        FullLog.LogD(TAG, "createStream(): Sending FCPublish command...");
        // transactionId == 3
        ...
        FullLog.LogD(TAG, "createStream(): Sending createStream command...");
        ChunkStreamInfo chunkStreamInfo =
                rtmpSessionInfo.getChunkStreamInfo(ChunkStreamInfo.RTMP_CID_OVER_CONNECTION);
        // transactionId == 4
    ...
    private void fmlePublish() {
        ...
        FullLog.LogD(TAG, "fmlePublish(): Sending publish command...");
    ...
    private void onMetaData() {
        ...
        FullLog.LogD(TAG, "onMetaData(): Sending empty onMetaData...");
        ...
        ecmaArray.setProperty("framerate", fps);
    ...
    private void closeStream() {
        ...
        FullLog.LogD(TAG, "closeStream(): setting current stream ID to 0");
    ...
    private void shutdown(boolean r) {
        if (socket != null) {
            ...
            if (socket != null){
                try {
                    socket.close();
                    FullLog.LogD(TAG, "socket closed");
    ...
    private void handleRxPacketLoop() {
        ...
        while (!Thread.interrupted()) {
            try {
                ...
                if (rtmpPacket != null) {
                    //FullLog.LogD();(TAG, "handleRxPacketLoop(): RTMP rx packet message type: " + rtmpPacket.getHeader().getMessageType());
                    switch (rtmpPacket.getHeader().getMessageType()) {
                        ...
                        case USER_CONTROL_MESSAGE:
                            ...
                            switch (user.getType()) {
                                ...
                                case PING_REQUEST:
                                    ...
                                    FullLog.LogD(TAG, "handleRxPacketLoop(): Sending PONG reply..");
                        ...
                        case WINDOW_ACKNOWLEDGEMENT_SIZE:
                            ...
                            FullLog.LogD(TAG, "handleRxPacketLoop(): Setting acknowledgement window size: " + size);
                        ...
                        case SET_PEER_BANDWIDTH:
                            ...
                            ChunkStreamInfo chunkStreamInfo =
                                    rtmpSessionInfo.getChunkStreamInfo(ChunkStreamInfo.RTMP_CID_PROTOCOL_CONTROL);
                            FullLog.LogD(TAG, "handleRxPacketLoop(): Send acknowledgement window size: "
        ...
        switch (commandName) {
            case "_error":
                try {
                    ...
                    if (description.contains("reason=authfailed")) {
                        ...
                        inputStream = new BufferedInputStream(socket.getInputStream());
                        outputStream = new BufferedOutputStream(socket.getOutputStream());
                        FullLog.LogD(TAG, "connect(): socket connection established, doing handshake...");
            ...
            case "_result":
                ...
                if ("connect".equals(method)) {
                    ...
                    FullLog.LogD(TAG, "handleRxInvoke(): Stream ID to publish: " + currentStreamId);
                    if (streamName != null && publishType != null) {
                        fmlePublish();
                    }
                } else if ("releaseStream".contains(method)) {
                    FullLog.LogD(TAG, "handleRxInvoke(): 'releaseStream'");
                } else if ("FCPublish".contains(method)) {
                    FullLog.LogD(TAG, "handleRxInvoke(): 'FCPublish'");
                } else {
                    Log.w(TAG, "handleRxInvoke(): '_result' message received for unknown method: " + method);
                }
                break;
            case "onBWDone":
                FullLog.LogD(TAG, "handleRxInvoke(): 'onBWDone'");
                break;
            case "onFCPublish":
                FullLog.LogD(TAG, "handleRxInvoke(): 'onFCPublish'");
                break;
            case "onStatus":
                String code =
                        ((AmfString) ((AmfObject) invoke.getData().get(1)).getProperty("code")).getValue();
                FullLog.LogD(TAG, "handleRxInvoke(): onStatus " + code);
