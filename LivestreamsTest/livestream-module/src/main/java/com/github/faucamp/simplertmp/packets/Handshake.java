...
public final class Handshake {
    ...
    public final void writeC1(OutputStream out) throws IOException {
        FullLog.LogD(TAG, "writeC1");
        FullLog.LogD(TAG, "writeC1(): Calculating digest offset");
        ...
        FullLog.LogD(TAG, "writeC1(): (real value of) digestOffset: " + digestOffset);

        FullLog.LogD(TAG, "writeC1(): recalculated digestOffset: " + absoluteDigestOffset);
        ...
        FullLog.LogD(TAG, "writeC1(): digestOffsetBytes: " + Util.toHexString(
        ...
        FullLog.LogD(TAG, "partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        FullLog.LogD(TAG, "writeC1(): Writing timestamp and Flash Player version");
        ...
        FullLog.LogD(TAG, "partAfterDigest(): size: " + partAfterDigest.length);
        random.nextBytes(partAfterDigest);

        // Set the offset byte
        FullLog.LogD(TAG, "copying digest offset bytes in partBeforeDigest");
        System.arraycopy(digestOffsetBytes, 0, partBeforeDigest, 772, 4);
        FullLog.LogD(TAG, "writeC1(): Calculating digest");
        ...
        FullLog.LogD(TAG, "writeC1(): writing C1 packet");
    ...
    public final void readS1(InputStream in) throws IOException {
        ...
        if (totalBytesRead != HANDSHAKE_SIZE) {
            ...
            FullLog.LogD(TAG, "readS1(): S1 total bytes read OK");
        }
    }
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            FullLog.LogD(TAG, "readS2(): S2 total bytes read OK");
        }
