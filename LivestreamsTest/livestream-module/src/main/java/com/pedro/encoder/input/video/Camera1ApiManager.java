...
//        cameraSelect = selectCameraFront();
    ...
    private void start() {
        ...
        try {
            ...
            int[] range = adaptFpsRange(preferFps, parameters.getSupportedPreviewFpsRange());
            fps = range[1] / 1000;
    ...
    private List<Camera.Size> getPreviewSize() {
        ...
        while (iterator.hasNext()) {
            ...
            if (size.width > maxSize.width || size.height > maxSize.height) {
                Log.i(TAG, size.width + "X" + size.height + ", not supported for com.sigma.live.encoder");
                iterator.remove();
            }
        }
        return previewSizes;
