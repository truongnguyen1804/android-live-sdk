...
import com.pedro.encoder.video.CustomDataVideo;
...
public abstract class Camera1Base
        implements GetAacData, GetCameraData, GetVideoData, GetMicrophoneData, CustomDataVideo {
     ...
     * Rotation with com.sigma.live.encoder is silence ignored in some devices.
     *
     * @param width            resolution in px.
     * @param height           resolution in px.
     * @param fps              frames per second of the stream.
     * @param bitrate          H264 in kb.
     * @param hardwareRotation true if you want rotate using com.sigma.live.encoder, false if you want rotate with
     ...
     *                         com.sigma.live.encoder is silence ignored in some devices.
     * @return true if success, false if you get a error (Normally because the com.sigma.live.encoder selected
     * doesn't support any configuration seated or your device hasn't a H264 com.sigma.live.encoder).
     */
    ...
    public boolean prepareVideo(int width, int height, int fps, int bitrate, boolean hardwareRotation,
                                int rotation) {
        FullLog.LogD("checkmResolution2: "+ width +" -- "+ height+" -- "+ fps);
     ...
     * @return true if success, false if you get a error (Normally because the com.sigma.live.encoder selected
     * doesn't support any configuration seated or your device hasn't a AAC com.sigma.live.encoder).
     */
...
//                glInterface.setRotation(360 - CameraHelper.getCameraOrientation(context));
                FullLog.LogD("prepareGlView: startPreview "+ CameraHelper.getRotationGl(CameraHelper.getCameraOrientation(context)));
            ...
            cameraManager.start(cameraFacing, width, height, fps);
            onPreview = true;
        } else {
    ...
    public int getFps() {
        return cameraManager.getFps();
    }

    public void startPreview(CameraHelper.Facing cameraFacing, int width, int height, int fps) {
        startPreview(cameraFacing, width, height, fps, CameraHelper.getCameraOrientation(context));
    }

    public void startPreview(CameraHelper.Facing cameraFacing, int fps) {
        startPreview(cameraFacing, 640, 480, fps);
    }

    public void startPreview(int width, int height, int fps) {
        startPreview(CameraHelper.Facing.BACK, width, height, fps);
    }

    public void startPreview(int fps) {
        startPreview(CameraHelper.Facing.BACK, fps);
    }
...
//            cameraManager.setSurfaceTexture(glInterfaceDouble.getSurfaceTexture());
    ...
    public void setCameraFace(int cameraFace) throws CameraOpenException{
        if (isStreaming() || onPreview) {
            cameraManager.setCameraFace(cameraFace);
        }
    }
    ...
    @Override
    public void onSetVideoThumbnail(Bitmap bitmap) {
        videoEncoder.prepareImage(bitmap);
    }
}