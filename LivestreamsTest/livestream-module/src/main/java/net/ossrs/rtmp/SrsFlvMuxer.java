...
public class SrsFlvMuxer {
    ...
    public void setFps(int fps) {
        publisher.setFps(fps);
        int limit = 6 * fps;
        BitrateControl.getInstance().setup(connectCheckerRtmp, limit);
        resizeFlvTagCache(limit);
    }
    ...
    public void reConnect(final long delay) {
        reTries--;
        Log.i(TAG, "SrsFlvMuxer closed reConnect");
        stop(null);
        ...
        if (frame.is_video()) {
            ...
            if (frame.is_keyframe()) {
                if (frameCount > 1) {
                    ...
                    Log.i(TAG,
                            String.format("worker: send frame type=%d, dts=%d, size=%dB, fps %d", frame.type, frame.dts,
                                    frame.flvTag.array().length, frameCount * 1000 / (now - lastTime)));
                    frameCount = 0;
                    lastTime = now;
                }
            }
    ...
    Runnable myRunable = null;

    /**
     * start to the remote SRS for remux.
     */
    public void start(final String rtmpUrl) {
        FullLog.LogE("reConnect: start 1");
//        if (myRunable!=null && worker!=null){
//            Handler handler1 = new Handler();
//            handler1.removeCallbacks(myRunable);
//        }
        if (worker != null) {
            FullLog.LogE("reConnect: start 10");
            worker.interrupt();
        }
        FullLog.LogE("reConnect: start 11");
        worker = new Thread(
                new Runnable() {
                    @Override
                    public void run() {
                        Process.setThreadPriority(Process.THREAD_PRIORITY_MORE_FAVORABLE);
                        connectCheckerRtmp.onConnectingRtmp();
                        if (!connect(rtmpUrl)) {
                            return;
                        }
                        FullLog.LogE("reConnect: start 2");
                        reTries = numRetry;
...
//                                FullLog.LogD(TAG+ "Skipping iteration, frame not null");
    ...
    public void stop() {
        Log.i(TAG, "SrsFlvMuxer closed connectCheckerRtmp");
    ...
    private class SrsFlv {
        ...
        private void writeH264SpsPps(int pts) {
            Log.i(TAG, "writeH264SpsPps: " + Pps + " -- " + Sps + " -- " + isPpsSpsSend + " -- " + pts);
        ...
        private int mDiscarded = 0;
        private long mLastDiscardNotify = 0;
        ...
        private void notifyDiscarded(int count) {
            ...
            if (mDiscarded > 0) {
                long now = System.currentTimeMillis();
                if (mDiscarded == 1)
                    mLastDiscardNotify = now;
                if (now - mLastDiscardNotify > 1000) {
                    mLastDiscardNotify = now;
                    Log.i(TAG, "Frame discarded " + mDiscarded);
                    mDiscarded = 0;
                }
            }
        }
